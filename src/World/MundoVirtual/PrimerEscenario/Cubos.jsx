...
const Cubos = () => {
    ...
    return (
        <>
            <mesh 
            ref={CubosRef} rotation-y={Math.PI / 2} position={[-3, 2, -1.6]} scale={1} 
            onClick={removeComponent1}>
                <boxGeometry args={[3, 3, 3]} />
                <meshBasicMaterial {...propsTexture} />
            </mesh>

            <mesh ref={CubosRef2} rotation-y={Math.PI / 2} position={[-3, 2, 1.6]} scale={1} 
            onClick={removeComponent2}>
                <boxGeometry args={[3, 3, 3]} />
                <meshBasicMaterial {...propsTexture} />
            </mesh>

            <mesh ref={CubosRef3} rotation-y={Math.PI / 2} position={[-3, 5.1, 1.6]} scale={1} 
            onClick={removeComponent3}>
                <boxGeometry args={[3, 3, 3]} />
                <meshBasicMaterial {...propsTexture} />
            </mesh>

            <mesh ref={CubosRef4} rotation-y={Math.PI / 2} position={[-3, 5.1, -1.6]} scale={1} 
