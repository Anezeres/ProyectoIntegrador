			...
			if (xanderRef && xanderBodyRef) {
				if (KeysPressing.size > 0) {
					...
					if (KeysPressing.has("w") && KeysPressing.has("s")) {
						...
						xanderRef.current.position.x +=
							Math.sin(xanderRef.current.rotation.y) * 0.015;
						xanderRef.current.position.z +=
							Math.cos(xanderRef.current.rotation.y) * 0.015;
						...
						xanderRef.current.position.x -=
							Math.sin(xanderRef.current.rotation.y) * 0.01;
						xanderRef.current.position.z -=
							Math.cos(xanderRef.current.rotation.y) * 0.01;
