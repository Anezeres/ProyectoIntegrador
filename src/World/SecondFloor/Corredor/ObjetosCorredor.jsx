...
const Objetos = (props) => {
    ...
    const moverSalsaTomate = () => {
        moveObjectToPositionSmoothly(salsaRef, [3.5, 2, -8.7], 1000, () => {
            // Esta función se ejecutará cuando la animación termine
        }, true);
    }

    const moverSalsaTomate2 = () => {
        moveObjectToPositionSmoothly(salsaRef2, [3.5, 2, -8.7], 1000, () => {
            // Esta función se ejecutará cuando la animación termine
        }, true);
    }

    const moverSalsaTomate3 = () => {
        moveObjectToPositionSmoothly(salsaRef3, [3.5, 2, -8.7], 1000, () => {
            // Esta función se ejecutará cuando la animación termine
        }, true);
    }

    const moverDulces = () => {
        moveObjectToPositionSmoothly(dulcesRef, [2.3, 0.77, -3], 1000, () => {
            // Esta función se ejecutará cuando la animación termine
        }, true);
    }
    ...
    return (
        <group {...props} dispose={null} >
            ...
            <RigidBody type="fixed" colliders={false}>
                <CuboidCollider args={[1.2, 1, 1]} position={[0, 0, -1.8]} />  {/* Mesa Blanca */}
                <CuboidCollider args={[0.3, 0.5, 0.3]} position={[-1.5, 0.7, -1.4]} /> {/* Silla */}
                <CuboidCollider args={[1.3, 0.5, 1]} position={[12.5, 0.5, -5.8]} /> {/* Mesa Azul */}
                <CuboidCollider args={[0.5, 1.5, 2]} position={[15, 0.8, -6]} /> {/* Televisor */}
                <CuboidCollider args={[0.6, 1.5, 2.7]} position={[3.6, 0.8, -6.6]} />  {/* Cocina */}
                <CuboidCollider args={[0.3, 0.7, 1]} position={[9, 0.8, 0]} />  {/* Piano */}
                <CuboidCollider args={[0.4, 1, 0.6]} position={[8.8, 0.8, 2.4]} /> {/* Casa Gato */}
                <CuboidCollider args={[4, 1, 0.1]} position={[11.7, 0.8, -3.1]} />  {/* Pared Tv */}
                <CuboidCollider args={[7, 1, 0.1]} position={[-2.2, 0.8, -3.1]} /> {/* Pared Cuadro */}
                <CuboidCollider args={[9.3, 1, 0.1]} position={[0, 0.8, 3.1]} /> {/* Pared Cuarto Abuela */}
                <CuboidCollider args={[0.1, 1, 3]} position={[-9.3, 0.8, 0]} /> {/* Pared Monitor */}
                <CuboidCollider args={[0.1, 1, 3]} position={[9.3, 0.8, 0]} /> {/* Pared Gato */}
                <CuboidCollider args={[0.1, 1, 3]} position={[3, 0.8, -6]} /> {/* Pared Nevera */}
